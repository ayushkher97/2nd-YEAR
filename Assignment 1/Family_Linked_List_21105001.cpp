#include <iostream>                   //Ayush Kher
using namespace std;                  //ECE,SID:21105001
...
class Node{
    ...
    string name;
    int age;
    Node*next; //A node pointer for the next node.
    Node*prev; //A node pointer for the previous node.
    //Calling constructor:
...
//FUNCTION TO INSERT DATA AT END OF THE LINKED LIST:
void append(Node* &head,string name,int age){
    //Making a node pointer and storing address of head in temp:
    Node*temp=head;
    //Making a new node and storing name and age in it:
    Node*new_node=new Node(name,age);
    //Inserting node in a empty list:
    if(temp==NULL){
        head=new_node;
    }
    //Inserting node in a non empty list:
    else{
...
//FUNCTION TO INSERT DATA AT START OF THE LINKED LIST:
void insert_at_head(Node*&head,string name,int age){
    //Making a Node pointer and storing address of new node in it:
    Node*new_node=new Node(name,age);
...
//FUNCTION TO DIPLAY DATA FROM START OF THE LINKED LIST:
...
//FUNCTION TO DELETE ANY INDEX(0,n-1) EXCLUDING THE LAST ELEMENT:
